...
function hexToRgb(hex) {
  ...
  if (hex.length === 8) {
    ...
    const a = (bigint & 255) / 255; // Нормализуем альфа-канал к диапазону [0, 1]
    return { r, g, b, a };
    ...
    const b = bigint & 255;
    return { r, g, b, a: 1 }; // Альфа-канал по умолчанию 1 (непрозрачный)
  } else {
...
colorText.on("save", (color) => {
  if (color) {
    const hexColor = color.toHEXA().toString();
    console.log("Цвет текста выбран:", hexColor);
    COLOR_TEXT = hexColor;
    updateCanvas();
  }
});
